...
df = pd.read_csv("cleaning_throughput_rows_sec.csv")
...
df["configuration"] = df.apply(
    lambda row: (
        ...
        else "Cloud environment - 1 node (AWS, no schema inference)" 
        if row["operation-system"] == "standard - 1 node (without schema inference)"
...
color_map = {
    ...
    "Cloud environment - 1 node (AWS, no schema inference)": "#9467bd",  # purple 
...
st.subheader("Throughput by System Configuration and Dataset Size")
...
fig_time = px.line(
    ...
    title = "Throughput Across Dataset Sizes",
    color_discrete_map = color_map,
    labels = {
        "slice_label": "Dataset Size (MiB)",
        "execution_time": "Throughput (rows/seconds)",
...
fig_time.update_layout(
    ...
    yaxis_tickformat=",",
    yaxis_title="Throughput (rows/seconds)",
    legend_title="Configuration"
)

max_val = df["throughput_rows_sec"].max()
upper = math.ceil(max_val / 100000) * 100000
